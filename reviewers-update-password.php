<?php
//function for changing password for author
session_start();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['change_password'])) {
    ...
    $queryPassword = "SELECT password FROM reviewers WHERE name='$userName'";
        ...
        if (password_verify($currentPassword, $currentPasswordHash)) {
            ...
            if ($currentPassword != $newPassword) {
                ...
                if (!empty($newPassword) && !empty($confirmPassword) && $newPassword === $confirmPassword) {
                    ...
                    $updateQuery = "UPDATE reviewers SET password='$hashedPassword' WHERE name='$userName'";
